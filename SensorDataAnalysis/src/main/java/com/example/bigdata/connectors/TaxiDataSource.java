...
public class TaxiDataSource implements SourceFunction<TaxiData> {
    ...
    @Override
    public void run(SourceContext<TaxiData> sourceContext) throws Exception {
        running = true;
                ...
                if (file.isFile() && file.getName().endsWith(".csv")) {
                    try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
                        ...
                        while ((line = reader.readLine()) != null && running) {
                            ...
                            try {
                                TaxiData taxiEvent = TaxiData.fromString(line);
